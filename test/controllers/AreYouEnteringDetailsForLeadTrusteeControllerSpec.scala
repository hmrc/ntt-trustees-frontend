...
class AreYouEnteringDetailsForLeadTrusteeControllerSpec extends SpecBase with MockitoSugar with NunjucksSupport with JsonMatchers {

  def onwardRoute = Call("GET", "/foo")

  val formProvider = new AreYouEnteringDetailsForLeadTrusteeFormProvider()
  val form = formProvider()

  lazy val areYouEnteringDetailsForLeadTrusteeRoute = routes.AreYouEnteringDetailsForLeadTrusteeController.onPageLoad(NormalMode).url

  "AreYouEnteringDetailsForLeadTrustee Controller" - {
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      val application = applicationBuilder(userAnswers = Some(emptyUserAnswers)).build()
      val request = FakeRequest(POST, areYouEnteringDetailsForLeadTrusteeRoute).withFormUrlEncodedBody(("value", ""))
